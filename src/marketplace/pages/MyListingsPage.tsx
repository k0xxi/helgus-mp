...
export function MyListingsPage() {
  ...
  const { user } = useAuth()
  const { listings, loading, togglePause, markAsSold, reactivateListing, deleteListing } = useMyListings(user?.id)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 overflow-x-auto">
        {(['all', 'active', 'paused', 'pending', 'sold'] as const).map((status) => (
          <button
            key={status}
                  ...
                  {listing.status === 'pending' && (
                    <>
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-700 hover:bg-amber-100 disabled:opacity-50 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400 dark:hover:bg-amber-900/30"
                        title="Verkauf abbrechen und reaktivieren"
                      >
                      ...
                      <button
                        ...
                        disabled={actionLoading === listing.id}
                        className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100 disabled:opacity-50 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30"
